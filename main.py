...
for page in range(40):
    ...
    for item in tqdm.tqdm(tags):
        ...
        resp_object = r.get(url_object, headers=us_ag)
        ...
        tag_price = soup_object.find(attrs={"data-qa":"vacancy-salary"}).find(attrs={"class":"bloko-header-section-2 bloko-header-section-2_lite"})
        
        tag_region = soup_object.find(attrs={"data-qa":"vacancy-view-location"})
       
        tag_op = soup_object.find(attrs= {"class":"wrapper-flat--H4DVL_qLjKLCo1sytcNI"}).find(attrs={"data-qa":"vacancy-experience"})
